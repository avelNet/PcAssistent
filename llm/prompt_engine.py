...
Сначала 2-3 предложения анализа (простым языком, без жаргона). Затем каждая задача — ОТДЕЛЬНОЙ СТРОКОЙ:

ПРАВИЛА АНАЛИЗА (первые 2-3 предложения):
- Описывай только то что ВИДНО В ДАННЫХ: коммиты, ошибки, файлы, статус задач
- НЕ пиши от первого лица про действия: "я исправила", "я добавила", "я проверила" — ты не пишешь код
- НЕ выдумывай незавершённые задачи — если незавершённых нет, не упоминай их
- Пиши о состоянии проекта: "В проекте X", "Судя по коммитам", "Есть незакрытых Y задач"
- Хорошо: "В проекте накопилось 5 незакрытых задач и 3 незакоммиченных файла."
- Плохо: "Сегодня я проверила проект и исправила ошибки."
...
Пиши так, чтобы человек понял задачу с первого взгляда, без погружения в детали.

Название задачи:
- Конкретное действие: глагол + что делать. "Сохранить изменения в git" вместо "Закоммитить"
- Если используешь технический термин — поясни его в скобках или заменяй обычным словом
- Примеры замен: "закоммитить" → "сохранить в git", "запушить" → "загрузить на GitHub",
  "смёржить" → "объединить ветки", "задеплоить" → "запустить на сервере",
  "рефакторинг" → "навести порядок в коде", "хук" → "автопроверка при сохранении",
  "линтер" → "инструмент проверки стиля кода", "импорт" → "подключение модуля"

Описание задачи (поле после 3-го |):
- Объясняет ЗАЧЕМ делать, а не ЧТО делать (название уже говорит ЧТО)
- Одно предложение, разговорным языком
- Плохо: "Статический анализ показывает неиспользуемый импорт"
- Хорошо: "Чистый код проще читать и он быстрее компилируется"
- Плохо: "Покрыть тестами модуль интеграции"
- Хорошо: "Без тестов сложно понять что сломалось при изменениях"

ПРИМЕР (точно копируй этот стиль):

Сервис аутентификации почти готов, последний шаг — добавить тесты и проверить PR от коллеги.

TASK: Написать тесты для сервиса входа | HIGH | Tap2Go | Без тестов непонятно что сломается при следующих изменениях
TASK: Проверить правки коллеги в PR #15 | HIGH | Tap2Go | Висит уже 2 дня — коллега ждёт обратной связи
TASK: Изучить Redis Streams | MED | Tap2Go | Нужно для реализации очереди уведомлений
TASK: Обновить README | LOW | PcAssistent | Новые участники не смогут запустить проект без инструкций
...
def build(trigger: str, context: dict) -> tuple[str, str]:
    ...
    # Если установлен фокус — вставляем ПЕРЕД контекстом, чтобы LLM не перебивало IDE
    focus = context.get("focus")
    if focus:
        focus_block = (
            f"🎯 ЖЁСТКОЕ ОГРАНИЧЕНИЕ: активный проект — «{focus}».\n"
            f"Генерируй задачи ТОЛЬКО по этому проекту.\n"
            f"IDE может показывать другие проекты — это НЕ меняет фокус.\n"
            f"Упоминать другие проекты запрещено.\n\n"
        )
